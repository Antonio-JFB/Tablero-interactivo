...
# Instanciamos el objeto OCR
ocr = CnOcr()  # Usa valores predeterminados para CnOcr
...
for filename in os.listdir(input_folder):
    ...
    if filename.endswith('.pdf'):
        ...
        if pdf_document.page_count > 0:
            ...
            # Usar CnOcr para extraer texto de la imagen
            out = ocr.ocr(image_path)

            # Filtrar solo el texto y mostrarlo
            extracted_text = "\n".join([item['text'] for item in out])

            # Filtrar solo las líneas que contienen entidades y números de homicidios usando la expresión regular
